...
| 티치 모드 ON | 드라이버가 `cannot control` → MoveIt/CAN 제어 전부 거부 | 세션 중 티치 버튼 금지. 눌렀으면 `/exit_teach_mode`(Empty, piper 계열) 또는 전원 재인가. **S-V1.7-3 은 teach 탈출 시 팔 드랍** → 받친 채로. 한 세션=수동 또는 CAN 하나만 |
